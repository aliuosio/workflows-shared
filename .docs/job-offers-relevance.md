...
- [AI Agent Pipeline Details](#ai-agent-pipeline-details)
...
### The Scout Agent
The Scout uses the TCREI framework (Task-Context-Role-Execute-Interrogate) to dynamically analyze job requirements. It extracts actual skills, experience levels, and domain expertise from job postings without relying on hardcoded assumptions.

### The Analyst Agent
The Analyst applies mathematical precision to qualitative assessments using a fixed 45/25/20/10 weighting formula. This ensures consistent, deterministic scoring across all job types.

### The Matchmaker Agent
The Matchmaker focuses solely on clean JSON formatting, preventing AI conversational artifacts from corrupting the structured output required for database storage.
